...
:arg_help
echo DESCRIPTION: %BASENAME% remove a path from the PATH environment variable
...
call "%DIRNAME%\lib\api.cmd" :edebug PATH_TO_REMOVE = '%PATH_TO_REMOVE%'

rem
rem check if path exist
rem use delayed expansion because PATH_TO_REMOVE may contain parenthesis and thus end the IF block prematurly
if exist "!PATH_TO_REMOVE!" (
    call "%DIRNAME%\lib\api.cmd" :ewarn Path '!PATH_TO_REMOVE!' still exists on current system !!
)
...
rem if VARIABLE is empty, nothing more to do
if "x!CURRENT_PATH!" == "x" goto :end
rem
rem check if path is already present
set PATH_TEST=!CURRENT_PATH:%PATH_TO_REMOVE%=!
call "%DIRNAME%\lib\api.cmd" :edebug PATH_TEST = '%PATH_TEST%'
rem call "%DIRNAME%\lib\api.cmd" :edebug PATH_TEST = %PATH_TEST%
rem do not use code block between parenthesis in case variables contains parenthesis
if "!CURRENT_PATH!" == "!PATH_TEST!" call "%DIRNAME%\lib\api.cmd" :ewarn Path '%PATH_TO_REMOVE%' is not present in %VARIABLE% variable
if "!CURRENT_PATH!" == "!PATH_TEST!" goto :end

:do_remove_path
rem remove path
set CURRENT_PATH=!CURRENT_PATH:%PATH_TO_REMOVE%=!
