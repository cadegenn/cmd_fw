...
rem 2014.11.24, DCA -	ajout variable API_PATH => chemin vers api.cmd et les outils détecté automatiquement
...
set THIS_BASENAME=%~nx0


rem title %0
call "%API_PATH%\api.cmd" :ebegin #include %THIS_BASENAME% - BEGIN
...
call "%API_PATH%\api.cmd" :edebug ARCH = %ARCH%
...
call "%API_PATH%\api.cmd" :edebug ProgFiles(x86) = %ProgFiles(x86)%
...
call "%API_PATH%\api.cmd" :edebug ProgFiles(x64) = %ProgFiles(x64)%
...
call "%API_PATH%\api.cmd" :edebug HKLM_SOFTWARE(x86) = %HKLM_SOFTWARE(x86)%
set HKLM_SOFTWARE(x64)=HKLM\Software
call "%API_PATH%\api.cmd" :edebug HKLM_SOFTWARE(x64) = %HKLM_SOFTWARE(x64)%

rem déclaration des HKLM_UNINSTALL
set HKLM_UNINSTALL(x86)=%HKLM_SOFTWARE(x86)%\Microsoft\Windows\CurrentVersion\Uninstall
call "%API_PATH%\api.cmd" :edebug HKLM_UNINSTALL(x86) = %HKLM_UNINSTALL(x86)%
set HKLM_UNINSTALL(x64)=%HKLM_SOFTWARE(x64)%\Microsoft\Windows\CurrentVersion\Uninstall
call "%API_PATH%\api.cmd" :edebug HKLM_UNINSTALL(x64) = %HKLM_UNINSTALL(x64)%

REM rem ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
REM rem Déclaration de binaires
REM rem
REM rem 7-zip au cas ou
REM set ZIP="%API_PATH%\7-Zip\%ARCH%\7z.exe"
REM call "%API_PATH%\api.cmd" :edebug ZIP = %ZIP%

REM rem SED
REM set SED=%API_PATH%\GnuWin32\bin\sed.exe
REM call "%API_PATH%\api.cmd" :edebug SED = %SED%

REM rem pour la suite, on a besoin de iconv
REM rem set ICONV="%SOFTWARE%\_Utils\GnuWin32\bin\iconv.exe"
REM set ICONV="%API_PATH%\GnuWin32\bin\iconv.exe"
REM call "%API_PATH%\api.cmd" :edebug ICONV = %ICONV%
...
REM call "%API_PATH%\api.cmd" :edebug CommonPrograms = %CommonPrograms%
...
REM call "%API_PATH%\api.cmd" :edebug CommonDesktop = %CommonDesktop%
...
REM call "%API_PATH%\api.cmd" :edebug CommonStartup = %CommonStartup%
...
REM call "%API_PATH%\api.cmd" :edebug CommonAppData = %CommonAppData%
...
REM call "%API_PATH%\api.cmd" :edebug CommonDocuments = %CommonDocuments%
...
rem call "%API_PATH%\api.cmd" :edebug Windows_ImageState = %Windows_ImageState%
...
REM call "%API_PATH%\api.cmd" :edebug UsersDir = %UsersDir%
...
for /f "tokens=1,2* delims==" %%i in ('set Win') do call "%API_PATH%\api.cmd" :edebug %%i = %%j
...
for /f "tokens=1,2* delims==" %%i in ('set Computer') do call "%API_PATH%\api.cmd" :edebug %%i = %%j
...
for /f "tokens=1,2* delims==" %%i in ('set BIOS') do call "%API_PATH%\api.cmd" :edebug %%i = %%j

rem :_end
call %API_PATH%\api.cmd :ebegin #include %THIS_BASENAME% - END
